...
public class AlienConvoy {
    ...
    /**
     * The `updateDirection()` method is updating the direction of the AlienConvoy
     * based on the position of
     * the leftmost and rightmost aliens in the convoy. It first filters the list of
     * aliens to only include
     * those that are part of the convoy, then finds the leftmost and rightmost
     * aliens using the `min()`
     * and `max()` methods respectively. If the rightmost alien is at the right edge
     * of the screen, the
     * direction is set to LEFT, and if the leftmost alien is at the left edge of
    ...
    public void updateDirection() {
        ...
        GameObject leftObj = Aliens.stream().filter(c -> c.partOfConvoy).min(Comparator.comparing(GameObject::getX))
                .get();
        GameObject rightObj = Aliens.stream().filter(c -> c.partOfConvoy).max(Comparator.comparing(GameObject::getX))
                .get();
    ...
    /**
     * This function drops an alien from either the left or right side of the screen
    ...
    /**
     * The `isStageCleared()` method is checking if the `Aliens` ArrayList is empty,
     * and returning a
     * boolean value indicating whether the stage is cleared. If the ArrayList is
     * empty, it means
    ...
    /**
     * This function moves a convoy of aliens either left or right by adjusting
     * their x-coordinates.
     */
    ...
    /**
     * Resets the state of the AlienConvoy.
     * Initializes the positions of different types of aliens.
     */
    public void reset() {
    ...
    /**
     * This method initializes a convoy of aliens by creating instances of different
     * alien
     * classes (`YellowShip`, `RedAlien`, `PurpleAlien`, and `GreenAlien`) and
     * adding them to the `Aliens`
     * ArrayList. The method takes an instance of the `PApplet` class as a
     * parameter, which is used to call
     * the `setup()` method on each alien instance to initialize its appearance and
     * behavior. The aliens
     * are positioned on the screen using specific x and y coordinates, which are
